...
onmessage = async (e: MessageEvent<WorkerMessage>) => {
  ...
  const { requirements, compiledListPagesTemplates } = Parser.extractDataRequirements(ast)

  // 4. Resolve modules with external data
  const resolved = await Parser.resolveModules(ast, {
    fetchListPages: async (query) => {
      ...
      return {
        ...
        site: {
          name: 'scp-wiki', title: "SCP Wiki", domain: "scp-wiki.wikidot.com"
        }
      }
    },
