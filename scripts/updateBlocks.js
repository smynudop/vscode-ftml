...
const blocktomllink =
	"https://raw.githubusercontent.com/scpwiki/wikijump/develop/ftml/conf/blocks.toml";
...
!(async () => {
	...
	let tmLangConfigRaw = fs.readFileSync(
		path.join(process.cwd(), "syntaxes/ftml.tmLanguage.yaml"),
		"utf-8",
	);
	let tmLangConfig = yaml.load(
		tmLangConfigRaw.replace(/\# \- include/g, "- include"),
	);
	for (const key in regexes) {
	...
	for (const key in regexes) {
		if (regexes[key].disabled) {
			newTmLangConfigRaw = newTmLangConfigRaw.replace(
				`- include: '#${kebabize(key)}'`,
				`# - include: '#${kebabize(key)}'`,
			);
		}
	}
